...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    var enemy1_target:SKShapeNode? = SKShapeNode()
    var enemy1_decide_to_chase:Bool = true
    
    var enemy2 = SKShapeNode(circleOfRadius: 20)
    var enemy2_target:SKShapeNode? = SKShapeNode()
    var enemy2_decide_to_chase:Bool = true
    ...
    func setupEnemy(){
        //enemy1
        enemy1.position = getRandomPosition()
        enemy1.strokeColor = SKColor.redColor()
        enemy1.fillColor = SKColor.blackColor()
        enemy1.lineWidth = 7
        enemy1.zPosition = 4
        
        enemy1.physicsBody = SKPhysicsBody(circleOfRadius: 20 * 0.95)
        enemy1.physicsBody!.affectedByGravity = false
        enemy1.physicsBody!.mass = 1
        enemy1.physicsBody!.dynamic = true
        enemy1.physicsBody!.categoryBitMask = PhysicsCategory.Enemy
        enemy1.physicsBody!.contactTestBitMask = PhysicsCategory.Food | PhysicsCategory.Player
        enemy1.physicsBody!.collisionBitMask = PhysicsCategory.VerticalEdge | PhysicsCategory.HorizontalEdge
        enemy1_target = player.centralCell
        enemy1_decide_to_chase = true
        
        //enemy2
        enemy2.position = getRandomPosition()
        enemy2.strokeColor = SKColor.redColor()
        enemy2.fillColor = SKColor.blackColor()
        enemy2.lineWidth = 7
        enemy2.zPosition = 4
        
        enemy2.physicsBody = SKPhysicsBody(circleOfRadius: 20 * 0.95)
        enemy2.physicsBody!.affectedByGravity = false
        enemy2.physicsBody!.mass = 1
        enemy2.physicsBody!.dynamic = true
        enemy2.physicsBody!.categoryBitMask = PhysicsCategory.Enemy
        enemy2.physicsBody!.contactTestBitMask = PhysicsCategory.Food | PhysicsCategory.Player
        enemy2.physicsBody!.collisionBitMask = PhysicsCategory.VerticalEdge | PhysicsCategory.HorizontalEdge
        enemy2_target = player.centralCell
        enemy2_decide_to_chase = true
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        case PhysicsCategory.Enemy | PhysicsCategory.Food:
            if(contact.bodyA.categoryBitMask == PhysicsCategory.Enemy){
                let enemy = contact.bodyA.node as! SKShapeNode
                let food = contact.bodyB.node as! SKShapeNode
                modList.removeObject(food)
                ...
                if(enemy == enemy1){
                    enemy1_target = find_closest_food(enemy1.position)
                }else if(enemy == enemy2){
                    enemy2_target = find_closest_food(enemy2.position)
                }
            ...
            //spawn new food
            let food = modController.createFood()
            food.physicsBody!.categoryBitMask = PhysicsCategory.Food
            food.physicsBody!.contactTestBitMask = PhysicsCategory.Player
            food.physicsBody!.collisionBitMask = PhysicsCategory.None
    ...
    func AIThinking()
    {
        ...
        if(enemy1.frame.width * 0.9 > player.centralCell!.frame.width ){
            if(enemy1_decide_to_chase){
                //chase player
                enemy1.physicsBody!.velocity = CGVector(
                    dx: enemy1_target!.position.x - enemy1.position.x,
                    dy: enemy1_target!.position.y - enemy1.position.y) //direction
            }else{
                //switch strategy
                enemy1_decide_to_chase = true
            ...
            //enemy should eat food
        }else{
            if(enemy1_decide_to_chase){
                //switch strategy
                enemy1_decide_to_chase = false
                
                //switch target to food
                enemy1_target = find_closest_food(enemy1.position)
            }
            //move to food
            enemy1.physicsBody!.velocity = CGVector(
        ...
        //enemy2 thinking
        //enemy should chase player
        if(enemy2.frame.width * 0.9 > player.centralCell!.frame.width ){
            if(enemy2_decide_to_chase){
                //chase player
                enemy2.physicsBody!.velocity = CGVector(
                    dx: enemy2_target!.position.x - enemy2.position.x,
                    dy: enemy2_target!.position.y - enemy2.position.y) //direction
    ...
    func find_closest_food(position : CGPoint) -> SKShapeNode{
        var shortest:CGFloat = 50000.0
        var shortest_index = 0
        var temp:CGFloat = 0.0
        var xDiff:CGFloat = 0.0
        var yDiff:CGFloat = 0.0
        
        for (index,node) in modList.enumerate(){
            xDiff = node.position.x - position.x
            yDiff = node.position.y - position.y
